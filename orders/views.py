...
def generateQRAndSave(request, form):
    ...
    constructed_url = "http://{}/orders/confirmation/{}".format(
        request.get_host(), form.data.get('csrfmiddlewaretoken'))
    ...
    image_url = "http://{}{}{}{}".format(request.get_host(),
                                         settings.MEDIA_URL, 'orders/', file_name)
...
def exportcsv(request):
    ...
    writer.writerow(['Name', 'Email', 'Event Date', 'Event Name', 'Phone',
                    'QR Code', 'Family Count', 'Prasad Count', 'Created On', 'Is Verified'])
    ords = orders.values_list('name', 'email', 'event_date', 'event_name', 'phone',
                              'qr_code_img_url', 'family_count', 'prasad_count', 'created_on', 'is_verified')
    for ord in ords:
        writer.writerow(ord)
    return response
